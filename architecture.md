...
### Node Registry & Self-Describing Nodes
The system follows a self-describing node architecture.

#### Each node type encapsulates:
- Metadata (label, constraints, entry behavior)
- Type-safe configuration schema
- Execution handler (pure function)
- UI renderer component

#### Configuration editor component
All node types are registered through a centralized Node Registry, which maps:
```
nodeType → nodeDefinition
```

### Registry Responsibilities
- Provide node metadata to the UI layer
- Provide execution handlers to the workflow engine
- Keep execution logic decoupled from Vue and Pinia
- The registry itself is framework-agnostic and contains no reactive state.

### Execution Engine Decoupling
The workflow engine is implemented as a pure TypeScript module.

It:
- Accepts a graph structure
- Uses the Node Registry to resolve execution handlers
- Executes deterministically
- Emits execution results without direct UI dependency

The engine:
- Does NOT import Vue
- Does NOT import Pinia
- Does NOT manipulate UI state

This ensures:
- Testability
- Separation of concerns

Future portability beyond Vue
...
This deployment strategy minimizes infrastructure complexity. For production-grade deployment, the application can be containerized using a lightweight Node.js image and deployed to a VM or Kubernetes cluster.
